...
describe('Create and Retrieve Files and Folders through Server to MongoDB', () => {
    ...
    describe('retrieveFilesFolders()', () => {
        ...
        it('Find the new nodes from the book shelf in MongoDB', async () => {
            ...
            Folder.find().count(function(err, count) {
                assert.equal(count, 1);
            });

            Folder.find({ name: 'Sample' }).then((mongoResponse) => {
                assert.isObject(mongoResponse);
                assert.equal(mongoResponse.name, 'Samples');
                assert.equal(Folder.convertPathToString(mongoResponse.path), 'd:/Backend/Nodejs/intranet.bookshelf.nodejs/test/sample-server/Books/Samples');
            });
        });  
    });
